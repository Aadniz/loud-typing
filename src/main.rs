...
fn validate_pitch_deviation(val: &str) -> Result<f32, String> {
    let pitch_deviation: f32 = val
        .parse()
        .map_err(|_| "Pitch deviation must be a number")?;
...
fn key_down(event: InputEvent, player_mutex: Arc<Mutex<SoundPlayer>>) {
    if let Ok(player) = player_mutex.lock() {
        let code = match event {
            InputEvent::Mouse(key) => match key.button {
                MouseButton::Left => 0,
                MouseButton::Right => 1,
            ...
            InputEvent::Keyboard(key) => key.code,
            _ => 0,
        };
        player.play(code);
    }
...
fn handle_event(event: InputEvent, player: Arc<Mutex<SoundPlayer>>) {
    match event {
        InputEvent::Mouse(key) => match key.status {
            Status::Pressed => key_down(InputEvent::Mouse(key), player),
            Status::Released => key_up(),
        },
        InputEvent::Keyboard(key) => match key.status {
...
fn main() {
    ...
    let player = SoundPlayer::new(paths)
        .unwrap()
    ...
    let callback: SourceCallback =
        Box::new(move |event| handle_event(event, Arc::clone(&player_mutex)));
